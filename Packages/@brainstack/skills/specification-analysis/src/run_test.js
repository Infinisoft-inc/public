...
// Define a MessageSource interface
class MessageSource {
  async receiveMessage() {
    // To be implemented by specific message source
  }
...
// Implement a ConsoleMessageSource class based on the MessageSource interface
class ConsoleMessageSource extends MessageSource {
  async receiveMessage() {
...
class InformationGatheringTask extends Task {
  constructor(apiKey) {
    super();
  ...
  async run(input) {
    const response = await this.openaiClient.createChatCompletion({
      model: "gpt-3.5-turbo",
      messages: this.messages.concat({ role: "user", content: input }),
    });
...
// Create the ConversationAssistant with the ConsoleMessageSource
const generateContentAssistant = new ConversationAssistant(APIKEY, consoleMessageSource);
generateContentAssistant.run();
