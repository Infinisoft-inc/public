...
class BridgeClient {
    ...
    constructor(options?: BridgeClientOptions) {
        ...
        this.hub = options?.hub || createEventHub({ source: '', logger: this.logger });

        this.logger.verbose(
            'BridgeClient initialized with URL:',
            'Reconnect Interval:', this.reconnectInterval,
        ...
        this.hub.on(
            ...
            (payload: any) => {
                if (this.ws?.readyState === WebSocket.OPEN) {
                    this.logger.verbose('Sending message to WebSocket server. Payload:', payload);
